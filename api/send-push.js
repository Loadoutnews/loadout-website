...
// Update-Kalender, Breaking News) — NICHT öffentlich zugänglich, sondern
// durch ein geheimes Passwort geschützt (PUSH_SECRET), damit nicht
// irgendwer beliebige Nachrichten an alle Nutzer:innen schicken kann.
//
// NEU: kann optional gezielt nur an Abonnent:innen senden, die das
// betroffene Spiel/die Kategorie in ihren Präferenzen ausgewählt haben
// ("Folge nur deinen Spielen"-Feature). Ohne games/categories im Aufruf
// (z. B. bei Breaking News oder generischen Sammel-Benachrichtigungen)
// wird weiterhin an ALLE gesendet — genau wie bisher. Auch Abonnent:innen
// OHNE gesetzte Präferenzen bekommen weiterhin ausnahmslos alles, damit
...
export default async function handler(request, response) {
  ...
  const { title, body, url, games, categories } = request.body || {};
  if (!title || !body) {
  ...
  try {
    ...
    await Promise.all(endpoints.map(async (endpoint) => {
      ...
      // Gezieltes Filtern nur, wenn BEIDES zutrifft: der Aufruf hat
      // überhaupt Ziel-Spiele/-Kategorien angegeben UND die/der
      // Abonnent:in hat überhaupt eigene Präferenzen gesetzt. Sonst (z. B.
      // Breaking News ohne games/categories, oder eine Person ohne
      // gesetzte Präferenzen) wird immer zugestellt.
